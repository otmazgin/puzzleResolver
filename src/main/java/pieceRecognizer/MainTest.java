...
public class MainTest {
	...
	try {
	    ...
	    PuzzlePieceDetector.pieceDetector(src, h_min, s_min, v_min, h_max, s_max, v_max, Pices);

	    for (int i = 0; i < Pices.size(); i++) {

		PuzzlePieceCornerDetector.cornerFindeByQuaters(Haris_blockSize, Haris_apertureSize, Haris_k,
			Haris_threashHold, Pices.get(i).getMask(), Pices.get(i).getRect(), new Mat(), Pices
